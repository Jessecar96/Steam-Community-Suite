...
if(!window.localStorage['scs-domains-time'] || !window.localStorage['scs-domains'] || (get_time() - window.localStorage['scs-domains-time']) > 7200){
	...
	$.getJSON("https://raw.githubusercontent.com/Jessecar96/Steam-Community-Suite/master/assets/domains.json", function parse_domains(data) {
		...
		window.localStorage['scs-domains-time'] = get_time();
		window.localStorage['scs-domains'] = JSON.stringify(data);
		log("Loaded & Cached JSON");
		main_script();
	});
}else{
	var data = JSON.parse(window.localStorage['scs-domains']);
	safe_domains = data.safe_domains;
	bad_domains = data.bad_domains;
	ignore_domains = data.ignore_domains;
